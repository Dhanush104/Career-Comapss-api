        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const [projectsRes, userProfileRes, analyticsRes, leaderboardRes, tasksRes, deadlinesRes] = await Promise.all([
                    ...
                    fetch('http://localhost:5000/api/analytics/leaderboard?type=xp&limit=5', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    }).catch(() => null),
                    fetch('http://localhost:5000/api/goals/today', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    }).catch(() => null),
                    fetch('http://localhost:5000/api/goals/deadlines', {
                        headers: { 'Authorization': `Bearer ${token}` }
                    }).catch(() => null)
                ]);
